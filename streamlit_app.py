...
with st.sidebar:
    ...
    if st.button("Load Patient", type="primary"):
        if health_id and len(health_id) >= 10:
            ...
            patient = mock.get(health_id, {"name": f"Member …{health_id[-4:]}", "age": 35, "sex": "—"})
...
if not st.session_state.current_patient:
    st.info("Load a patient to begin")
    st.stop()

p = st.session_state.current_patient
st.success(f"**Active patient:** {p['name']} | {p.get('age')}y | {p.get('sex')} | …{p['health_id'][-4:]}")
...
def get_sats_priority():
    ...
    if score >= 7:  return "ORANGE",  "#ff9100"
    if score >= 4:  return "YELLOW",  "#ffff00"
    return "GREEN", "#00e676"

if st.button("Calculate SATS Priority", type="primary"):
    priority, colour = get_sats_priority()
...
# ————————————————————————
# PDF report (ReportLab – 100 % reliable)
# ————————————————————————
def create_pdf():
    buffer = BytesIO()
    ...
    story.append(Paragraph("Priority      : " + st.session_state.get("last_priority", "Not calculated"), styles["Normal"]))
    story.append(Spacer(1, 12))
    story.append(Paragraph("Chief complaint : " + (chief or "—"), styles["Normal"]))
    story.append(Paragraph("Symptoms       : " + (symptoms or "—"), styles["Normal"]))
    story.append(Paragraph("Vitals         : RR " + str(resp_rate) + " | HR " + str(heart_rate) + " | BP " + str(systolic_bp) + " | Temp " + str(temp) + "°C", styles["Normal"]))
